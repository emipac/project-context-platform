...
class LightRagEngine(Protocol):
    ...
    async def search(self, project_id: str, query: str, filters: dict[str, Any]) -> list[dict[str, Any]]: ...
...
def find_current_record_for_path(manifest: dict[str, Any], source_path: str) -> dict[str, Any] | None:
...
class ContractLightRagEngine:
    ...
    async def ingest(self, project_id: str, paths: list[str], mode: str, documents: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for source_path in paths:
            ...
            chunks.append({
                ...
                "heading": document.get("heading") if document else None,
                "stable_ids": document.get("stable_ids", []) if document else [],
                "content": document.get("content") if document else f"Indexed by LightRAG sidecar contract: {source_path}",
        ...
        return {"indexed": len(paths), "warnings": []}
    ...
    async def search(self, project_id: str, query: str, filters: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [chunk for chunk in ranked if score(chunk, terms) > 0 or not terms][:limit]

    async def get_spec_context(self, project_id: str, spec_id: str, include_neighbors: bool) -> list[dict[str, Any]]:
        chunks = self._load_chunks(project_id)
        matches = [chunk for chunk in chunks if spec_id in chunk["source_path"] or spec_id in chunk.get("stable_ids", [])]
    ...
    async def get_related_code(self, project_id: str, query: str, filters: dict[str, Any]) -> list[dict[str, Any]]:
        terms = terms_for(query)
        chunks = [chunk for chunk in self._load_chunks(project_id) if chunk["document_type"] in {"code", "test"}]
        ranked = sorted(chunks, key=lambda chunk: score(chunk, terms), reverse=True)
        limit = int(filters.get("limit", 10))
        return [chunk for chunk in ranked if score(chunk, terms) > 0 or not terms][:limit]
...
class CoreLightRagEngine:
    ...
    async def ingest(self, project_id: str, paths: list[str], mode: str, documents: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for source_path in paths:
            ...
            if not content:
                warnings.append(f"Skipped empty document: {source_path}")
                continue
            content_hash = hashlib.sha256(content.encode("utf-8")).hexdigest()
            previous = find_current_record_for_path(manifest, source_path)
            if previous and previous.get("content_hash") == content_hash:
                continue
            if previous:
                await self._delete_document_by_id(rag, str(previous["doc_id"]), warnings)
                mark_manifest_record_stale_by_doc_id(manifest, str(previous["doc_id"]), now, "replaced")

            doc_id = hashlib.sha256(f"{project_id}:{source_path}:{content_hash}".encode("utf-8")).hexdigest()
            records_to_insert.append({
                ...
                "stable_ids": document.get("stable_ids", []),
                "content": content,
                "document_type": document_type(source_path),
    ...
    async def search(self, project_id: str, query: str, filters: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            response = await asyncio.to_thread(self._query, rag, query)
        except Exception:
            response = ""
    ...
    async def get_spec_context(self, project_id: str, spec_id: str, include_neighbors: bool) -> list[dict[str, Any]]:
        namespace = self._namespace(project_id)
        manifest = self._load_manifest(namespace)
        matches = [record for record in current_records(manifest) if spec_id in record["source_path"] or spec_id in record.get("stable_ids", [])]
        if include_neighbors:
            paths = {record["source_path"] for record in matches}
            matches = [record for record in current_records(manifest) if record["source_path"] in paths]
        return [to_chunk(record) for record in matches]

    async def get_related_code(self, project_id: str, query: str, filters: dict[str, Any]) -> list[dict[str, Any]]:
        filters = {**filters, "document_types": ["code", "test"]}
        return await self.search(project_id, query, filters)
    ...
    async def get_document(self, project_id: str, selector: dict[str, str | None]) -> list[dict[str, Any]]:
        ...
        return [to_chunk(record) for record in sorted(matches, key=lambda item: item["source_path"])]
    ...
    def _query(self, rag: Any, query: str) -> str:
        from lightrag import QueryParam

        kw_sets = (
            dict(mode=self.settings.query_mode, only_need_context=True, top_k=self.settings.top_k, chunk_top_k=self.settings.chunk_top_k, max_total_tokens=self.settings.max_total_tokens),
            dict(mode=self.settings.query_mode, only_need_context=True, top_k=self.settings.top_k, chunk_top_k=self.settings.chunk_top_k),
            dict(mode=self.settings.query_mode, only_need_context=True),
            dict(mode=self.settings.query_mode),
        )
        param: Any = None
        for kw in kw_sets:
            try:
                ...
                continue
        if param is None:
            param = QueryParam(mode=self.settings.query_mode)
